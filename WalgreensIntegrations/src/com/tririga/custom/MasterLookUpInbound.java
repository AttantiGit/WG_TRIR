...
public class MasterLookUpInbound implements CustomBusinessConnectTask{
	...
	public boolean execute(TririgaWS tws, long userId, Record[] records) {
        ...
        try{
    		String integration = "HRIS Organization Inbound";
    			...
    			for(int i=0;i<configRecordData.length();i++ ){
    				...
    				if(IntegrationName.equalsIgnoreCase(integration)){
    					...
    					while(itr.hasNext()){    						
				        	...
				        	log.info (" Before Sections of cstHRISOrganizationDTO ");
				        	
				        	IntegrationRecord integrationRecordHRISOrganization = new IntegrationRecord();
				        	integrationRecordHRISOrganization.setId(-1l);
				        	integrationRecordHRISOrganization.setSections(sectionsPaymentInboundDTO);
					             
				        	 log.info (" Before cstDraft of cstHRISOrganizationDTO ");
				        	 integrationRecordHRISOrganization.setActionName("cstDraft");
				        	 integrationRecordHRISOrganization.setModuleId(tws.getModuleId("cstIntegration"));
					        
				           // log.info ("ModuleID"+tws.getModuleId("triIntermediate"));
					        log.info ("setObjectTypeId cstHRISOrganizationDTO ="+tws.getObjectTypeId("cstIntegration", "cstHRISOrganizationDTO"));
					            
					        integrationRecordHRISOrganization.setObjectTypeId(tws.getObjectTypeId("cstIntegration", "cstHRISOrganizationDTO"));
					        integrationRecordHRISOrganization.setObjectTypeName("cstHRISOrganizationDTO");
					        
					        try {
								
						        ResponseHelperHeader responseHelperHeaderPayment =  tws.saveRecord( new IntegrationRecord[] { integrationRecordHRISOrganization });
								...
								{
									log.info (this.getClass()+">>>>>>>>>> cstHRISOrganizationDTO Call Successfull");
						        }
						        }catch(Exception e){
						        	log.info (this.getClass()+">>>>>>>>>> cstHRISOrganizationDTO Call Failed");
									e.printStackTrace();
								}
    					}  
    					br.close();
