...
public class MDGVendorInbound implements CustomBusinessConnectTask{
	...
	public boolean execute(TririgaWS tws, long userId, Record[] records) {
        ...
        try{
    		String integration = "MDG Vendor Inbound";
    			...
    			for(int i=0;i<configRecordData.length();i++ ){
    				...
    				if(IntegrationName.equalsIgnoreCase(integration)){
    					...
    					while(itr.hasNext()){    						
				        	...
				        	log.info (" Before Sections of cstHRISTitleDTO ");
				        	
				        	IntegrationRecord integrationRecordHRISTitle = new IntegrationRecord();
				        	integrationRecordHRISTitle.setId(-1l);
				        	integrationRecordHRISTitle.setSections(sectionsPaymentInboundDTO);
					             
				        	 log.info (" Before cstDraft of cstHRISTitleDTO ");
				        	 integrationRecordHRISTitle.setActionName("cstDraft");
				        	 integrationRecordHRISTitle.setModuleId(tws.getModuleId("cstIntegration"));
					        
				           // log.info ("ModuleID"+tws.getModuleId("triIntermediate"));
					        log.info ("setObjectTypeId cstHRISTitleDTO ="+tws.getObjectTypeId("cstIntegration", "cstHRISTitleDTO"));
					            
					        integrationRecordHRISTitle.setObjectTypeId(tws.getObjectTypeId("cstIntegration", "cstHRISTitleDTO"));
					        integrationRecordHRISTitle.setObjectTypeName("cstHRISTitleDTO");
					        
					        try {
								
						        ResponseHelperHeader responseHelperHeaderPayment =  tws.saveRecord( new IntegrationRecord[] { integrationRecordHRISTitle });
								...
								{
									log.info (this.getClass()+">>>>>>>>>> cstHRISTitleDTO Call Successfull");
						        }
						        }catch(Exception e){
						        	log.info (this.getClass()+">>>>>>>>>> cstHRISTitleDTO Call Failed");
									e.printStackTrace();
								}
    					}
    					br.close();
